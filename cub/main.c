...
void raycasting(t_game *game) {
    ...
    for (int x = 0; x < SCREEN_WIDTH; x++) {
        // Cálculo de dirección del rayo (igual que antes)
        ...
        // Distancia perpendicular (igual que antes)
        double perpWallDist = (side == 0) ? (sideDistX - deltaDistX) : (sideDistY - deltaDistY);

        // Altura del muro (igual que antes)
        ...
        // --- CAMBIO PRINCIPAL: Eliminamos las correcciones de textura invertida ---
        double wallX;
        ...
        int texX = (int)(wallX * (double)texWidth); // <- Sin corrección de espejo

        // Dibujamos la columna de píxeles
